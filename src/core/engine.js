...
const Engine = {
  ...
  resize() {
    ...
    const pixels = r.width * r.height * dpr * dpr;
  ...
  tick(dt, now) {
    ...
    try {
      Effects.draw(g, 'back');
      vis.draw(g);
      Effects.draw(g, 'front');
    } catch (error) {
      if (this.onErr) this.onErr(vis.id, error.message || String(error));
    }
