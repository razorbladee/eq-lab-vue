...
const props = defineProps({
  recording: [Boolean, Object],
  countdown: [Number, Object],
  error: [String, Object],
  start: Function,
  stop: Function,
  cancel: Function,
...
<template>
  <details class="sec recorder-panel" open>
    ...
    <div class="sec-body panel-body">
      ...
      <label
        ><span class="plabel"
          >FPS <span class="pval">{{ fps }}</span></span
        ><select v-model.number="fps" :disabled="isRecording">
      ...
      <label
        ><span class="plabel"
          >Bitrate <span class="pval">{{ bitrateLabel }}</span></span
        ><input
      ...
      <label
        ><span class="plabel"
          >Автостоп <span class="pval">{{ duration ? `${duration} сек.` : 'Выкл.' }}</span></span
        ><select v-model.number="duration" :disabled="isRecording">
      ...
      <div class="grid grid-cols-2 gap-2">
        <button
          ...
          :disabled="isRecording"
          @click.prevent.stop="begin"
        >
          Запись</button
        ><button type="button" class="btn" :disabled="!isRecording" @click.prevent.stop="stop">
          Сохранить WebM
        </button>
      </div>
