...
globalThis.document = {
  createElement: (tag) =>
    tag === 'canvas' ? { width: 1, height: 1, getContext: () => context } : {},
};
...
for (const visualizer of VIS) {
  if (!visualizer.id || !visualizer.name || typeof visualizer.draw !== 'function')
    throw new Error(`Invalid visualizer: ${visualizer.id || 'unknown'}`);
...
for (let i = 0; i < Audio.bands.length; i++) Audio.bands[i] = (Math.sin(i * 0.13) + 1) * 0.35;
const waterfall = VMAP.waterfall;
const p = Object.fromEntries(
  Object.entries(waterfall.params).map(([key, schema]) => [key, schema.def]),
);
const g = {
  ctx: context,
  w: 960,
