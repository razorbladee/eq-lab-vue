...
const Effects = {
  items: structuredClone(defaults), media: structuredClone(mediaDefaults), states: Object.create(null), selected: 'snow',
  ...
  setAsset(slot, file) {
    ...
    asset.muted = true; asset.loop = true; asset.playsInline = true;
    const ready = () => { this.media[slot] = { ...this.media[slot], enabled: true, url, type: file.type, asset }; this.states['media:' + slot] = { url, asset, ready: true }; if (isVideo) asset.play().catch(() => {}); };
    asset.onload = ready; asset.onloadeddata = ready; asset.onerror = () => { this.media[slot].enabled = false; URL.revokeObjectURL(url); }; asset.src = url;
  },
  draw(g, phase) {
    for (const [id, p] of Object.entries(this.items)) {
      ...
      if (id === 'aurora') drawAurora(g, p); else { if (!st.seeded || st.x.length !== n) seed(st, n, g.w, g.h); if (id === 'snow') drawSnow(g, p, st, n); if (id === 'rain') drawRain(g, p, st, n); if (id === 'bokeh') drawBokeh(g, p, st, n); if (id === 'fireflies') drawFireflies(g, p, st, n); if (id === 'dust') drawDust(g, p, st, n); }
    }
    g.ctx.globalAlpha = 1; g.ctx.globalCompositeOperation = 'source-over';
  },
  drawMedia(g, slot) {
    const media = this.media[slot], asset = media?.asset || this.states['media:' + slot]?.asset;
    if (!media?.enabled || !asset) return;
    const isVideo = media.type.startsWith('video/'), ready = isVideo ? asset.readyState >= 2 : asset.complete && asset.naturalWidth > 0;
    if (!ready) return;
    const ratio = isVideo ? asset.videoWidth / asset.videoHeight : asset.naturalWidth / asset.naturalHeight;
    if (!Number.isFinite(ratio) || ratio <= 0) return;
    const box = mediaBox(g.w, g.h, media, ratio), ctx = g.ctx;
    ctx.save(); ctx.globalAlpha = clamp(Number(media.opacity) || 0, 0, 1); ctx.globalCompositeOperation = 'source-over'; ctx.drawImage(asset, box.x, box.y, box.w, box.h); ctx.restore();
  },
};
function mediaBox(w, h, m, ratio) { const bw = w * m.width / 100, bh = h * m.height / 100, x = w * m.x / 100 - bw / 2, y = h * m.y / 100 - bh / 2; if (m.fit === 'stretch') return { x, y, w: bw, h: bh }; const contain = m.fit === 'contain', boxRatio = bw / bh; let dw = bw, dh = bh; if ((ratio > boxRatio) === contain) dh = bw / ratio; else dw = bh * ratio; return { x: x + (bw - dw) / 2, y: y + (bh - dh) / 2, w: dw, h: dh }; }
...
function drawAurora(g, p) { const c = g.ctx; c.save(); c.globalCompositeOperation = 'lighter'; for (let l = 0; l < p.amount; l++) { const f = l / Math.max(1, p.amount - 1); c.beginPath(); for (let i = 0; i <= 80; i++) { const x = i / 80, y = g.h * (.2 + f * .34) + Math.sin(x * 5 + g.t * p.speed + l) * g.h * (.045 + .06 * g.A.b(x)); i ? c.lineTo(x * g.w, y) : c.moveTo(x * g.w, y); } c.lineTo(g.w, g.h * .56); c.lineTo(0, g.h * .56); c.closePath(); color(c, p.color); c.globalAlpha = p.opacity * (.04 + .05 * (1 - f)); c.fill(); } c.restore(); }
