...
const Engine = {
  ...
  mount(canvas) {
    ...
    this.g = { ctx: this.ctx, w: 0, h: 0, cx: 0, cy: 0, t: 0, dt: 0.016, q: 1, A: Audio, S: null, p: null, st: null, bg: '#0d0c12', col: x => Palette.at(x), rgba: (x, a) => Palette.rgba(x, a), sprite: x => Palette.sprite(x), alpha: a => { this.ctx.globalAlpha = clamp(a, 0, 1); }, add: () => { this.ctx.globalCompositeOperation = 'lighter'; }, norm: () => { this.ctx.globalCompositeOperation = 'source-over'; this.ctx.globalAlpha = 1; } };
  },
  resize() {
    const c = this.canvas, r = c?.getBoundingClientRect(); if (!c || !r || r.width < 2 || r.height < 2) return;
    const maxPixels = 2.6e6; let dpr = Math.min(window.devicePixelRatio || 1, 2); const pixels = r.width * r.height * dpr * dpr; if (pixels > maxPixels) dpr *= Math.sqrt(maxPixels / pixels);
    const width = Math.max(2, Math.round(r.width * dpr)), height = Math.max(2, Math.round(r.height * dpr)); this.DPR = dpr; this.W = r.width; this.H = r.height; this.g.w = r.width; this.g.h = r.height; this.g.cx = r.width / 2; this.g.cy = r.height / 2;
    if (c.width === width && c.height === height) return; c.width = width; c.height = height; this.states = Object.create(null); Effects.states = Object.create(null); Palette.sprites.fill(null); this.paintBg();
  },
  ...
  tick(dt, now) {
    ...
    const bg = S.canvasBg === 'theme' ? (document.documentElement.classList.contains('dark') ? '#12111a' : '#f2efe8') : (this.bgOf[S.canvasBg] || '#0d0c12');
    this.g.bg = bg;
    ...
    const vis = VMAP[S.preset] || VIS[0], g = this.g; g.dt = dt; g.t += dt * S.speed; g.q = this.q; g.S = S; g.p = P; g.st = this.state(vis.id);
    ctx.save(); ctx.translate(W / 2 + (S.posX - 50) * W / 100, H / 2 + (S.posY - 50) * H / 100); ctx.scale(S.zoomW, S.zoomH); ctx.translate(-W / 2, -H / 2); ctx.lineJoin = 'round'; ctx.lineCap = 'butt'; ctx.lineWidth = 1; ctx.textAlign = 'left'; ctx.textBaseline = 'alphabetic'; ctx.font = '400 12px "Space Mono",ui-monospace,monospace'; ctx.imageSmoothingEnabled = true;
    const safe = (fn, id) => { try { fn(); } catch (error) { if (this.onErr) this.onErr(id, error.message || String(error)); } finally { ctx.globalAlpha = 1; ctx.globalCompositeOperation = 'source-over'; ctx.filter = 'none'; } };
    safe(() => Effects.drawMedia(g, 'background'), 'background-media');
    safe(() => Effects.draw(g, 'back'), 'background-effects');
    safe(() => vis.draw(g), vis.id);
    safe(() => Effects.draw(g, 'front'), 'foreground-effects');
    safe(() => Effects.drawMedia(g, 'cover'), 'cover-media');
    ctx.restore(); ctx.globalCompositeOperation = 'source-over'; ctx.globalAlpha = 1; ctx.filter = 'none';
  },
};
export { Engine };
