...
const Palette = {
  mode: 'duo', c1: '#ff5a2d', c2: '#3ea8ff', cycle: 0,
  hue: 0, key: '', rev: 0,
  r: new Uint8Array(STOPS), g: new Uint8Array(STOPS), b: new Uint8Array(STOPS),
  str: new Array(STOPS).fill(null),
  sprites: new Array(SPRITES).fill(null),

  build(mode, c1, c2, hueOffset) {
    // Прокрутка оттенка живёт только в режиме «спектр». Иначе ключ менялся бы
    // каждый кадр и LUT со спрайтами пересобирались бы вхолостую.
    const hue = mode === 'spectrum' ? (hueOffset | 0) : 0;
    const key = mode + c1 + c2 + hue;
    if (key === this.key) return;
    this.key = key; this.mode = mode; this.c1 = c1; this.c2 = c2; this.hue = hue; this.rev++;
    const A = hexToRgb(c1), B = hexToRgb(c2);
    for (let i = 0; i < STOPS; i++) {
      const t = i / (STOPS - 1);
      let c;
      if (mode === 'single') c = A;
      else if (mode === 'spectrum') c = hslToRgb(t * 300 + hue, .92, .61);
      else if (mode === 'split') c = t < .5 ? A : B;
      else {
        // Дуо через квадратичное смешивание — меньше «грязи» в середине
        const m = t * t * (3 - 2 * t);
  ...
  /** Спрайт свечения вместо shadowBlur: ~40× быстрее в циклах. */
  sprite(x) {
    const k = clamp((x * (SPRITES - 1)) | 0, 0, SPRITES - 1);
    if (this.sprites[k]) return this.sprites[k];
    const s = 64, cv = document.createElement('canvas');
    cv.width = cv.height = s;
    const c = cv.getContext('2d');
    const i = clamp(((k / (SPRITES - 1)) * (STOPS - 1)) | 0, 0, STOPS - 1);
...
export { Palette, clamp, lerp, hexToRgb, hslToRgb, STOPS, SPRITES };
