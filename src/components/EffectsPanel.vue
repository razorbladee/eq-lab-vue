...
const effect = computed(() => Effects.items[selected.value]);
...
const update = (key, value) => { effect.value[key] = value; Effects.states = Object.create(null); };
const file = (slot, event) => {
  const source = event.target.files?.[0]; if (!source) return;
  const url = URL.createObjectURL(source), isVideo = source.type.startsWith('video/');
  const asset = isVideo ? document.createElement('video') : new Image();
  asset.muted = true; asset.loop = true; asset.playsInline = true;
  const ready = () => { Effects.media[slot] = { ...Effects.media[slot], enabled: true, url, type: source.type, asset }; if (isVideo) asset.play().catch(() => {}); };
  asset.onload = ready; asset.onloadeddata = ready; asset.src = url;
};
</script>
<template>
